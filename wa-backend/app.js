...
app.get('/user', function (req, res) {
  ...
  const { username, password } = req.body;
  console.log(username, password)
  connection.query(`SELECT u.id, u.name, u.username, u.location, r.name as rolename from user u  inner join role r on u.roleId = r.id;`, function(err, rows, fields) {
    if (!err) {
      if(typeof rows[0] != 'undefined'){
...
app.post('/user', function (req, res) {
  const { roleId, name, username, password, location } = req.body;
  connection.query(`insert into user (roleId, name, username, password, location) values  (${roleId}, '${name}', '${username}', MD5('${password}'), '${location}');`, 
...
app.post('/category', function (req, res) {
  const { name } = req.body;
  connection.query(`insert into category (name) values ('${name}');`, 
  function(err, result, fields) {
    if(err){
      return res.json({message: "Something went wrong", error: true}).status(500);
    }
...
app.delete('/category/:id', function (req, res) {
  const { id } = req.params;
  connection.query(`delete from category where id=${id};`, function(err, result, fields) {
    if(err){
      console.log(err);
      return res.json({message: "Something went wrong", error: true}).status(500);
    }
...
app.get('/sub-category', function (req, res) {
  // don't impl
  const { skip, limit } = req.query;
  connection.query(`select s.id, s.name, c.name as categoryName from subCategory s inner join category c on s.categoryId = c.id;`, function(err, rows, fields) {
    if(err){
      return res.json({message: "Something went wrong", error: true}).status(500);
    }
    res.json({message: "fetched", data: rows, error: false}).status(200);
...
app.get('/sub-categoryAll/:id', function (req, res) {
  const { id } = req.params;
  connection.query(`select s.id, s.name, c.name as categoryName from subCategory s inner join category c on s.categoryId = c.id where s.id=${id};`, function(err, rows, fields) {
    if(err){
      return res.json({message: "Something went wrong", error: true}).status(500);
    }
    res.json({message: "fetched", data: rows, error: false}).status(200);
...
app.get('/sub-category/:id', function (req, res) {
  const { id } = req.params;
  connection.query(`select * from subCategory where id=${id};`, function(err, rows, fields) {
    if(err){
      return res.json({message: "Something went wrong", error: true}).status(500);
    }
    res.json({message: "fetched", data: rows, error: false}).status(200);
...
app.post('/sub-category', function (req, res) {
  const { name, categoryId } = req.body;
  connection.query(`insert into subCategory (name, categoryId) values ('${name}', ${categoryId});`, 
  function(err, result, fields) {
    if(err){
      return res.json({message: "Something went wrong", error: true}).status(500);
    }
...
app.delete('/sub-category/:id', function (req, res) {
  const { id } = req.params;
  connection.query(`delete from subCategory where id=${id};`, function(err, result, fields) {
    if(err){
      console.log(err);
      return res.json({message: "Something went wrong", error: true}).status(500);
    }
...
app.get('/item', function (req, res) {
  ...
  connection.query(`select 
      ...
      inner join subCategory s  on i.subCategoryId=s.id `, function(err, rows, fields) {
    if(err){
      return res.json({message: "Something went wrong", error: true}).status(500);
    }
    res.json({message: "fetched", data: rows, error: false}).status(200);
...
app.delete('/item/:id', function (req, res) {
  const { id } = req.params;
  connection.query(`delete from items where id=${id};`, function(err, result, fields) {
    if(err){
      console.log(err);
      return res.json({message: "Something went wrong", error: true}).status(500);
    }
...
app.get('/warehouse-item/:warehouseId', function (req, res) {
  ...
  connection.query(`select 
      ...
      w.price, 
      w.quantity, 
      DATE_FORMAT(w.expiryDate, "%Y-%m-%d") as expiryDate,  
      w.remainingQuantity, 
      i.name, 
      c.name as category, 
